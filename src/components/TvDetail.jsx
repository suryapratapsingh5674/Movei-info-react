...
function TvDetail() {
  ...
  document.title = "Tv Info | Details"
 ...
 return info ? (
    <div style={{
      ...
      <h1 className='text-5xl text-white font-bold'>{info.detail.orignal_name || info.detail.title || info.detail.orignal_title || info.detail.name } <small className='text-2xl font-bold text-zinc-200'>({info.detail.first_air_date.split("-")[0]})</small></h1>
      ...
      <h1 className='text-2xl text-white mb-3 mt-5'>Tv Translated</h1>
      ...
      <h1 className='text-3xl font-bold text-white mb-8'>Seasons</h1>
      <div className='min-w-[15vw] flex gap-6 overflow-auto'>
        {info.detail.seasons.map((d, i) => (
          <div key={i} className='' >
            <img className="min-w-[12vw] h-[30vh] object-cover rounded-lg" src={ d.poster_path || d.profile_path ? `https://image.tmdb.org/t/p/original/${ d.poster_path || d.profile_path}` : noimage} alt="" />
